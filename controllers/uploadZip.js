...
const uploadZip = async (req, res) => {
  ...
  let { walletAddress, privateKey, field_of_study, domain, method, is_data_clean, dataset_name } =
    req.body;

  if (!walletAddress) {
    return res.status(400).json({
      message:
        "Wallet address, and field_of_study, domain, method, is_data_clean, dataset_name are required.",
    });
  }
  ...
  try {
    ...
    if (parseInt(walletBalance) < parseInt(transaction.reward)) {
      ...
      await fetch(`http://localhost:1984/mint/${walletAddress}/${requiredFunds}`, {
        method: "GET",
      });
