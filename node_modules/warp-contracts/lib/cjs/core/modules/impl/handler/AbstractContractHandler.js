...
class AbstractContractHandler {
    ...
    assignWrite(executionContext) {
        this.swGlobal.contracts.write = async (contractTxId, input, throwOnError) => {
            ...
            if (!activeTx.dry &&
                ...
                throw new Error(`No interact write tag for interaction ${activeTx.id}: ${JSON.stringify(debugData)} \n ${JSON.stringify(activeTx)}`);
            }
            ...
            const result = await calleeContract.applyInput(input, activeTx, executionContext.signal);
            this.logger.debug('Cache result?:', !activeTx.dry);
            const shouldAutoThrow = result.type !== 'ok' && effectiveThrowOnError && (!activeTx.dry || (activeTx.dry && activeTx.strict));
    ...
    assignViewContractState(executionContext) {
        this.swGlobal.contracts.viewContractState = async (contractTxId, input) => {
            ...
            return await childContract.viewStateForTx(input, this.swGlobal._activeTx, executionContext.signal);
        };
    }
